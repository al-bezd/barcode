  ...
  var form_select_user = new Vue({
    ...
    methods: {
      ...
      get_user_cordova() {
        ...
        const options = {
          method: 'get',
          data: { "ID": this.barcode.replace(" ", "") },
        }

        cordova.plugin.http.sendRequest(url_to_base + '/barcode2020/hs/barcode/get_user', options,
           (response)=> {
            RemoveData('scaningN')
            RemoveData('scaningN_free')
            if (response.РезультатПроверки) {
              SetBarcods();// UPDATE BARCODE
              SetData("current_user", response)
              SetData("user_documents", [])
              form_select_user.go_button = true
              form_menu.fio = response
              form_menu.show()
      ...
      get_user_ajax() {
        // не используется
        var request = $.ajax({
          url: url_to_base + '/barcode2020/hs/barcode/get_user',
          method: "GET",
          data: { "ID": PodgotovitBarcode(this,this.barcode) },
        })
      ...
      Scan() {
        ...
        cordova.plugins.barcodeScanner.scan(
           ...
           (error)=> {
            swal("Scanning failed: " + JSON.stringify(error))
          }
        )
      },
      close() {
